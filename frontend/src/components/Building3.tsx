...
const Building3: React.FC<Building3Props> = ({
  bookingsByRoom,
  selections,
  onSelectSlot,
  onBook,
  ...
  return (
    <div className="container" ref={containerRef} id={building3.id}>
      <h1>{building3.title}</h1>
      {building3.subtitle && <h2>{building3.subtitle}</h2>}
      <table className="room-table">
        <tbody>
          {(building3.rooms as RoomData[][]).map((row, rowIndex) => (
            <tr key={rowIndex}>
              {row.map((room) => (
