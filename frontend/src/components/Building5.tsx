...
const Building5: React.FC<Building5Props> = ({
  bookingsByRoom,
  selections,
  onSelectSlot,
  onBook,
  ...
  return (
    <div className="container" ref={containerRef} id={building5.id}>
      <h1>{building5.title}</h1>
      {building5.subtitle && <h2>{building5.subtitle}</h2>}
      <table className="room-table">
        <tbody>
          {(building5.rooms as RoomData[][]).map((row, rowIndex) => (
            <tr key={rowIndex}>
              {row.map((room) => (
